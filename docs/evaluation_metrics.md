...
The committed experiment export is generated by `src/experiment_runner.py`
against every discoverable stored scenario. With the current datasets, this is
94 scenarios total. Unit tests for metrics export into temporary directories so
the committed full-scenario reports are not overwritten during normal test runs.

The current committed artifacts show AgentShield at 0.0% ASR/FNR, 100.0% DSR,
100.0% BTSR, and 100.0% policy compliance across the 94 stored scenarios. The
prompt-only guardrail remains useful as a baseline: it catches obvious attacks
but still reports 74.0% ASR on the same corpus.
