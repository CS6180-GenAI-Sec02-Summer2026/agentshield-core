...
approach is concrete: the broader adversarial corpus exposed policy blind spots
that the smaller evaluation set missed, and it now serves as regression coverage
for the hardened firewall (see [`failure_analysis.md`](failure_analysis.md)).
...
that exposed the earlier policy blind spots and now validate the expanded rule
coverage.
...
The payoff of red-teaming: the expanded adversarial corpus surfaced missing
coverage in state-changing tools, public issue creation, sensitive file reads,
and external-sharing edge cases. After policy hardening, the same corpus now
validates 85/85 report-corpus agreement. Full analysis and the current
per-example table are in [`failure_analysis.md`](failure_analysis.md) and
